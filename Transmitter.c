...
if((64|PIND)==PIND) {PD6p=1;} else {PD6p=0;} //(энкодер подключен к 5 и 6 ножке порта D)
	...
	{if (speeder<32696)	{speeder= speeder+268;}else {speeder= 32697;}// для изменения положения нуля
	} 
	// decrement
	...
	{
		if (speeder>32696)	{speeder=32696;}
		if (speeder>804)
		{speeder= speeder-268;
		}
			}
...
if (speeder >=32697)
{ Sot=0; Des=0; Ed=Decoding[0]; stop=1;}// Перекинуть ноль с перед еденицы за 120. 
else
{
